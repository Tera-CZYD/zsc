...
class AnnouncementImagesController extends AppController {
  ...
  public function add() {
    ...
    if ($this->request->is(['post', 'ajax']) && $this->request->is('json')) {
      ...
      $id = @$requestData[0]['announcement_id'];
      ...
      $entities = $this->AnnouncementImages->newEntities($requestData);

      if ($this->AnnouncementImages->saveMany($entities)) {
