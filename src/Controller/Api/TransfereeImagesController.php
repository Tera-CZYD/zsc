...
class TransfereeImagesController extends AppController {
  ...
  public function add() {
    ...
    if ($this->request->is(['post', 'ajax']) && $this->request->is('json')) {
      ...
      $id = @$requestData[0]['transferee_id'];
      ...
      $entities = $this->TransfereeImages->newEntities($requestData);

      if ($this->TransfereeImages->saveMany($entities)) {
